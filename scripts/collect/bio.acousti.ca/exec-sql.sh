...
rsync scripts/field-usage.sh sp-bio-01:.
...
ssh -t sp-bio-01 "
	./field-usage.sh \"sudo docker exec -i scratchpads.apache drush\" > /tmp/field-usage.sql
	cat /tmp/field-usage.sql | sudo docker exec -i scratchpads.apache drush sqlc --extra=-fsN > /tmp/usage.txt
	cd /var/lib
